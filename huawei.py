...
class HuaWeiPush:
    ...
    def get_app(cls, appid):
        ...
        app = {}
        app["timestamp"] = now
        app["hw_appid"] = config.HW_APPID
        app["hw_app_secret"] = config.HW_APP_SECRET
        app["appid"] = appid
    ...
    def get_access_token(cls, hw_appid, hw_app_secret):
        ...
        resp = requests.post(LOGIN_URL, data=data, headers=headers, verify=False, timeout=60)
    ...
    def send(cls, access_token, device_token, title, content):
        ...
        headers = {"Content-Type":"application/x-www-form-urlencoded"}
        resp = cls.session.post(HUAWEI_URL, data=data, headers=headers, verify=False, timeout=60)
        if resp.status_code != 200:
...
if __name__ == "__main__":
    ...
    #APP_ID = 10448374
    #APP_SECRET = "85jseu03cqgitfoj6x0idyyxdhq1vvoy"
    #token = "0A00000590C458302000000854000001"
